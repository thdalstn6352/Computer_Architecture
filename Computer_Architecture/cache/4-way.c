...
int FindOldest(int index, int addr);
...
int ReadMem(int addr)
{
	unsigned int tag = (addr & 0xfffffc00) >>10;
	unsigned int index = (addr & 0x000003c0) >> 6;
	unsigned int offset = (addr & 0x0000003f);
	int dest_addr = (addr & 0xffffffc0);

	for(way = 0; way < 4; way++)
	{
		if(cache[way][index].valid == 0)
		{
		...
		int old_way = FindOldest(index, addr);

		cache[old_way][index].tag = tag;
		cache[old_way][index].valid = 1;
		cache[old_way][index].dirty = 0;

		miss_num++;
		Memory_access_num++;

		return cache[old_way][index].data[offset/4];
}
...
int WriteMem(int addr, int value)
{
	...
	cache[old_way][index].data[offset/4] = value;
	cache[old_way][index].tag = tag;
	cache[old_way][index].valid = 1;
	cache[old_way][index].dirty = 1;
...
void StoreMem(int drain_addr, int index)
{
	for(int i=0; i<16; i++)
	{
		memory[(drain_addr/4) + i] = cache[way][index].data[i];
	}
...
{	for(int i=0; i<16; i++)
	{
		cache[way][index].data[i] = memory[(dest_addr/4) + i];
	}
}
...
int FindOldest(int index, int addr)
{
	...
	if(cache[oldest_way][index].Sca == 0)
	{
		int drain_addr = (cache[oldest_way][index].tag << 10) | (index << 6);;
		int dest_addr = (addr & 0xffffffc0);

		if(cache[oldest_way][index].dirty == 1)
		{
			for(int i=0; i<16; i++)
			{
				memory[(drain_addr/4) + i] = cache[oldest_way][index].data[i];
			}
		}
	
		for(int i=0; i<16; i++)
		{
			cache[oldest_way][index].data[i] = memory[(dest_addr/4) + i];
		}
	...
	else if(cache[oldest_way][index].Sca == 1)
	{
		cache[oldest_way][index].Sca = 0;
