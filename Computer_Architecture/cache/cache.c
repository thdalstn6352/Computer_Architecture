	...
	else
	{
		...
		{	
			...
			case 0x4 :
				...
				{
						pc = IDEX_input_latch.pc4 + 4 + Address.Branch_address;
						Branch_satisfy_num++;
...
void Set_zero()
{
	...
	memset(cache, 0, sizeof(Cache_));
}

void Jump_addr()
{
	Address.Jump_address = ((IDEX_latch[0].pc4 + 4) & 0xf0000000) | (IDEX_latch[0].val_Rdata.address << 2);
//	pc = Address.Jump_address;
}
...
int ReadMem(int addr)
{
	unsigned int tag = (addr & 0xfffff000) >>12;
	unsigned int index = (addr & 0x00000fc0) >> 6;
	unsigned int offset = (addr & 0x0000003f);
	int dest_addr = (addr & 0xffffffc0);
	int drain_addr = (cache[index].tag << 12) | (index << 6); //tag + index
